...
def clean_df(df: pd.DataFrame) -> pd.DataFrame:
    ...
    # Обработка timestamp колонок (особое внимание для scheduled_at)
...
def extract(start_date: Optional[datetime] = None, end_date: Optional[datetime] = None):
    """Извлечение данных за период по scheduled_at"""
    end = end_date or datetime.now()
    start = start_date or (end - timedelta(days=DAYS))

    with pg_connection("api") as conn:
        df = pd.read_sql(
            f"SELECT * FROM {config.SCHEMA_SOURCE}.{TABLE} WHERE scheduled_at BETWEEN %s AND %s",
            conn,
            params=[start, end]
        )

    print(f"✓ Извлечено {len(df)} записей очереди повторных попыток за {start.date()} - {end.date()}")
...
def to_clickhouse(start_date: datetime, end_date: datetime):
    ...
    with engine.connect() as conn:
        # Используем scheduled_at для фильтрации
        df = pd.read_sql(
            f"SELECT * FROM {config.SCHEMA_FINAL}.{TABLE} WHERE scheduled_at BETWEEN %s AND %s",
            conn.connection,
...
def transaction_retry_queue_flow(start_date: Optional[datetime] = None, end_date: Optional[datetime] = None):
    ...
    print(f"\n✅ Загружено {len(df)} записей очереди за {s_date.date()} - {e_date.date()}\n")
